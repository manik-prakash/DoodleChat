...
wss.on('connection', async function connection(ws, request) {
  ...
  // Queue to hold messages until user is ready
  const messageQueue: string[] = [];
  let userReady = false;
  let user: User | null = null;

  // Set up message handler IMMEDIATELY to capture all messages
  ws.on('message', async function message(data) {
    const dataStr = data.toString();

    if (!userReady || !user) {
      // Queue the message until user is ready
...
// Separate function to process messages
    ...
    switch (parsedData.type) {
      case "join_room": {
        ...
        try {
          ...
          // Send confirmation to the user with room details
